...
static uint32_t proxPressStart[_PROX_BUTTONS];
static uint8_t  proxState[_PROX_BUTTONS];
...
bool mpr121Init()
{
  ...
  for(int i=0; i<_PROX_BUTTONS; i++) proxState[i] = PS_INIT;
...
void mpr121UpdateStates()
{
  ...
  for (uint8_t i=0; i<_PROX_BUTTONS; i++) 
  {
    ...
    if ((currtouched & _BV(i)) && !(lasttouched & _BV(i)) ) 
    {
      ...
      if (i==_CAP_SW_DOWN)  digitalWrite(_B0_LED, HIGH);
      if (i==_CAP_SW_UP)    digitalWrite(_B1_LED, HIGH);
    } // state changed
  }
  ...
  for (uint8_t i=0; i<_PROX_BUTTONS; i++)
  {
    ...
    if (!(currtouched & _BV(i)) && (lasttouched & _BV(i)) ) 
    {
      if (i==_CAP_SW_DOWN)  digitalWrite(_B0_LED, LOW);
      if (i==_CAP_SW_UP)    digitalWrite(_B1_LED, LOW);
...
void mpr121Handle()
{
  if ( (proxState[_CAP_SW_DOWN] == PS_LONG_PRESS) && (proxState[_CAP_SW_UP] == PS_LONG_PRESS) )
  {
    ...
    if (   ((millis() -proxPressStart[_CAP_SW_DOWN]) >= _PROX_VERY_LONG_RELEASE )
        && ((millis() -proxPressStart[_CAP_SW_UP]) >= _PROX_VERY_LONG_RELEASE )
       )
    {
